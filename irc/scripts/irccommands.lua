	...
	function ()
		...
		addIRCCommandHandler("!players",
			function (server,channel)
				if getPlayerCount() == 0 then
					...
					ircSay(channel,"There are "..getPlayerCount().." players in-game currently")
				end
			end
		)
		...
		addIRCCommandHandler("!country",
			function (server,channel,user,command,name)
				...
				local UCDcountry = getResourceFromName("UCDcountry")
				if UCDcountry and getResourceState(UCDcountry) == "running" then
					local player = getPlayerFromPartialName(name)
					if player then
						local abbrev, country = exports.UCDcountry:getCountry(getPlayerIP(player))
						if country then
							ircSay(channel, tostring(getPlayerName(player)).."'s country: "..tostring(country)..", "..tostring(abbrev))
						else
					...
					ircNotice(user, "'UCDcountry' is not running")
				end
			end
		)
	end
